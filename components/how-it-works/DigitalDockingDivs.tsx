...
const DEFAULT_STEPS: Step[] = [
  {
    id: "plan",
    title: "Plan",
    body: "Share your travel dates, destinations, and preferences.",
  },
  {
    id: "curate",
    title: "Curate",
    body: "Within 24–48 hours, we’ll handcraft the perfect itinerary.",
  },
  {
    id: "confirm",
    title: "Confirm",
    body: "Receive your quote, settle the invoice, and lock in your trip.",
  },
  {
    id: "travel",
    title: "Travel",
    body: "Bags packed, memories waiting—your journey begins.",
  },
];
...
export default function DigitalDockingDivs({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (plane && track && trackGlow) {
        ...
        const travel = () =>
          Math.max(0, track.offsetHeight - plane.offsetHeight);
        ...
        gsap.to(plane, {
          ...
          onUpdate: () => {
            const planeTipY =
              plane.getBoundingClientRect().top + plane.offsetHeight * 0.5;
  ...
  useEffect(() => {
    ...
    cardRefs.current.forEach(
      (c) => c && ((c.style.opacity = "1"), (c.style.transform = "none"))
    );
    textRefs.current.forEach(
      (t) => t && ((t.style.opacity = "1"), (t.style.transform = "none"))
    );
    const plane = planeRef.current;
    const trackGlow = trackGlowRef.current;
  ...
  return (
    <section
      ...
      className="relative isolate overflow-hidden bg-black px-4 py-16 text-primary-foreground sm:px-6 md:px-10"
    >
      <div className="absolute inset-x-0 top-0 h-32" aria-hidden />
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <div className="relative mt-12 md:mt-16">
          ...
          <div
            aria-hidden
            className="pointer-events-none absolute inset-y-14 left-1/2 hidden -translate-x-1/2 md:flex"
          >
          ...
          {/* STEPS */}
          <ol className="relative flex flex-col gap-12 pl-12 sm:pl-16 md:pl-0 md:gap-24">
                ...
                <li key={step.id} className="relative">
                  <div
                    className={`relative w-full pt-6 text-left md:pt-12 md:w-[min(520px,calc(50%-3.5rem))] ${
                      alignRight
                        ? "md:ml-auto md:pl-16 md:text-right"
                        : "md:mr-auto md:pr-16"
                    }`}
                  >
                    <div
                      ...
                      className="relative z-10 rounded-2xl bg-[#F8F7F3] px-6 py-7 text-foreground backdrop-blur-lg ring-1 ring-black/5 shadow-sm transition-shadow duration-300 focus-within:ring-2 focus-within:ring-primary/30 hover:shadow-md md:px-8 md:py-10"
                    >
                      <div
                        ref={(el) => {
                      ...
                      >
                        ...
                        <h3 className="text-pretty text-xl font-semibold leading-tight tracking-tight text-foreground sm:text-2xl md:text-3xl">
                          {step.title}
                        </h3>
                        <span
                          aria-hidden
                          className={`inline-block h-0.5 w-12 rounded-full bg-gradient-to-r from-primary/60 to-primary/20 ${
                            alignRight ? "md:self-end" : "md:self-start"
                          }`}
                        />
                        <p className="max-w-prose text-sm leading-relaxed text-muted-foreground sm:text-base sm:mt-1.5">
                          {step.body}
                        </p>
                      </div>
                    </div>
